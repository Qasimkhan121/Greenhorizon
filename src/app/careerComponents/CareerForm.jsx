...
import { getVisitorAPIInfo, getCountryCode } from "../utils/utils";
import { upload } from "../_icons/icons";

function CareerForm({ formName, setLoad, styles = {}, type }) {
  ...
  const validate = (data = formData) => {
    ...
    } else if (!/\.(pdf|doc|docx)$/i.test(data.resume.name)) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // http://nextapi.scizers.com/greenHorizon
      ...
      >
        <div className="flex flex-col w-full gap-6">
          ...
          <div className="text-start">
            <div className="w-full inputStyle flex justify-between items-center">
              {formData.resume ? (
                <label className="block mb-2 ">{formData.resume.name}</label>
              ) : (
                <label className="block mb-2 ">{t("resume")} </label>
              )}
              <input
                type="file"
                accept=".pdf,.doc,.docx"
              ...
              <label
                ...
                className="cursor-pointer  py-2 px-4 rounded transition"
              >
                {upload}
              </label>
            </div>
          ...
          <textarea
            ...
            onChange={handleChange}
            name="message"
            className="w-full textareaStyle text-white flex items-center bg-transparent "
            placeholder="Message*"
          ></textarea>
        </div>
        ...
        <div className="flex text-start lg:text-center items-center gap-2 mt-6">
          <div>
            ...
            />
            <span
              className={`w-6 h-6 flex items-center justify-center text-sm border transition-all duration-200
                ${
                  formData.agreeToTerms
                    ? type === "light"
                      ? "bg-[#f1e0c2] text-green"
